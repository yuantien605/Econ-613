...
#2.3 
#using lm
reg1 <- lm(wage ~ age + as.factor(year), data = Mind2)
summary(reg1)$coefficients
#the age coefficient is now -202.5028
...
solve(t(X) %*% X) %*% t(X) %*% Mind2$wage #-202.522

#using plm
install.packages("plm")
library(plm)

reg2 <- plm(wage ~ age, data = Mind2, model = "within", index = "year")
summary(reg2) #-202.5028
...
#3.4 !!!!!
# Error in optim(searchv, fn = ProLike2, method = "BFGS", control = list(trace = 6, : initial value in 'vmmin' is not finite
...
results4[ which(results4$V4 == min(results4$V4)), ] 
...
#failed attempt to create multiple year vectors 
#for (i in 2006:2015){
#  paste("y", i, sep = "") = M6[,i-1996]   
#}
...
result  = optim(searchv, fn = ProLike3, method = "BFGS", 
                control = list(trace = 6, REPORT = 1, maxit = 8000),
               age = age, y6 = y6, y7 = y7, y8 = y8, y9 = y9, y10 = y10, 
               y11 = y11, y12 = y12, y13 = y13, y14 = y14, y15 = y15,
