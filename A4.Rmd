...
```{r}
selection <- function(par, X, Z, y, observed_index) {

  #first stage: estimated coefficients:intercept, work_exp, schooling
  
  firstpar  = par[1:3] 
  lp_probit = Z %*% firstpar
  
  #second stage: intercept, work_exp, schooling, inverse mills
  beta  = par[4:7]
  lp_lm = X %*% beta
  ...
  loglike = sum(log(1-pnorm(lp_probit[!observed_index]))) +
    - log(sigma) +
