...
Mig <- M1 %>%
  ...
  filter(move_in == TRUE)
# table(Mig$empstat, Mig$year, Mig$idmen)

Mig2 <- Mig[order(Mig$idmen, Mig$idind, Mig$empstat),]
Mig2 <- Mig2 %>%
...
try <- function (x) {
  Mig2 %>%
    filter(idind == x) %>%
    mutate(change = length(unique(empstat)) >= 2) # bc 2 represents change in condition (1 is no change)
}
...
tryMatrix$change2[i] = sample(unlist(tryMatrix$change[i]), 1) #I use sample because "change" is either all true or all false 
}
#right now, I have compiled "whether an individual changes their profession" in change 2
...
entry_exit(M1$idind[678])


a <- lapply(unique(M1$idind), entry_exit)
b <- bind_rows(a)
y <- b[!duplicated(b),] #parse out duplicated individual observation
...
#Report your final result as a table in proportions. I will do this in the PDF
